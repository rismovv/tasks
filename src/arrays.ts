...
export function injectPositive(values: number[]): number[] {
    const firstNegIdx = values.findIndex((v) => v < 0);
    if (firstNegIdx === -1) {
        const sum = values.reduce((acc, curr) => acc + curr, 0);
        return values.concat(sum);
    } else {
        const beforeNeg = values.slice(0, firstNegIdx);
        const sum = beforeNeg.reduce((acc, curr) => acc + curr, 0);
        return beforeNeg
            .concat(values[firstNegIdx], sum)
            .concat(values.slice(firstNegIdx + 1));
    }
}
